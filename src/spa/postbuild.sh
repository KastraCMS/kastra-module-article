...
echo '1/4 Create "res" directory'
mkdir ../../../res
# Find all files, excluding (through 'grep'):
# - '.',
# - the newly created directory './Res/'
# - all content for the directory'./static/'
# Move all matches to the directory './Res/'
echo '2/4 Move relevant files'
cp -v -f -R ./js ../../../res
cp -v -f -R ./css ../../../res
# Browse into './Res/' directory
cd ../../../res

find ./js -type f | grep -E "\.[0-9a-z]{8}\." | rename -f "s/.[a-z0-9]{8}.chunk/.article/g"
find ./css -type f | grep -E "\.[0-9a-z]{8}\." | rename -f "s/.[a-z0-9]{8}.chunk/.article/g"
...
find . -type f -maxdepth 1 | LC_ALL=C xargs -I{} sed -i.backup -e 's,static/,../../res/static/,g' {}
